...
# Apps menu
show_apps() {
    ~/.config/launcher-aditaya/files/launchers/type-4/launcher.sh
}
...
# Setup menu - ENHANCED
show_setup() {
    MANAGER=$(echo -e "󰏔 Install\n󰆴 Remove\n󰚰 Update\n󰄯 Post Install" | rofi -dmenu -i -p "Setup")
    
    case "$MANAGER" in
        *"Post Install")
            bash "$(dirname "$0")/04.arch-boki-rofi-post-install.sh"
            ;;
        *"Install"*)
            show_install
            ;;
...
# Maintenance
show_maintenance() {
    MANAGER=$(echo -e "󱐋 Performance\n󰖩 WiFi\n󰂯 Bluetooth\n󱡶 Services\n󰅬 Scripts" | rofi -dmenu -i -p "Setup")
    case "$MANAGER" in
        *"Performance"*)
            show_performance
            ;;
...
# Time Tracker
#show_timetracker() {
#    ~/.config/rofi/rofi-binoymanoj/scripts/timetracker.sh
#}
...
# Search
show_search() {
    ~/.config/rofi/rofi-binoymanoj/scripts/choose_search_engine.sh
}
...
show_projects() {
    ...
    dir_map["Cconfig"]="$HOME/.config"
    dir_map["Local-bin"]="$HOME/.local/bin"
    dir_map["Local-share"]="$HOME/.local/share"
    dir_map["DATA"]="$HOME/DATA"
    dir_map["Public"]="$HOME/Public"
    dir_map["Templates"]="$HOME/Templates"
    
    for dir in "${!dir_map[@]}"; do
        if [ -d "${dir_map[$dir]}" ]; then
            dir_list+=("$dir")
        fi
    done
    ...
    SELECTED=$(printf '%s\n' "${dir_list[@]}" | rofi -dmenu -i -p "📁 Select Project Directory")
    
    if [ -z "$SELECTED" ]; then
        return
    fi
    
    SELECTED_PATH="${dir_map[$SELECTED]}"
    
    if [ -d "$SELECTED_PATH" ]; then
        thunar "$SELECTED_PATH" &
        notify-send "Projects" "Opening $SELECTED in Thunar"
    else
...
# Keybinds menu
show_keybinds() {
    ~/.config/rofi/rofi-binoymanoj/scripts/keymap-menu.sh
}
...
# System menu
show_system() {
    ~/.config/rofi/rofi-binoymanoj/scripts/power-menu.sh
}
